...
static void udhcp_run_script(struct dhcp_packet *packet, const char *name)
{
	...
	while (_tmp) {
		...
		value = index(_tmp, '=');
		value++;
		if (!strncmp(key, "interface", strlen("interface")))
			arr_strcpy(ifc.devname, value);
		if (!strncmp(key, "ip", strlen("ip")))
			inet_aton(value, &ifc.ip);
		if (!strncmp(key, "subnet", strlen("subnet")))
			inet_aton(value, &ifc.mask);
		if (!strncmp(key, "router", strlen("router")))
			inet_aton(value, &ifc.gw);
		if (strncmp(key, "dns", strlen("dns"))== 0) {
			char * dns = NULL;
			dns = strtok(value, " ");
			if (dns)
				inet_aton(dns, &ifc.dns[0]);	
		}
		_tmp =  envp[i++];
	}
	...
	if (strcmp(name, "bound") == 0)
		mu_msg(IF_MOD_IFACE_IP_UP, &ifc, sizeof(ifc), NULL, 0);
	else if (strcmp(name, "deconfig") == 0)
		mu_msg(IF_MOD_IFACE_IP_DOWN, &ifc, sizeof(ifc), NULL, 0);
	/* call script */
