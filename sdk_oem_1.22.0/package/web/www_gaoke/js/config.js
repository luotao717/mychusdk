...
function setPPPOE(account, passwd, mac, mtu, dns, dns1, clone) {
    ...
    if (currentMode != 2 || mac != myMac || account != myAccount || passwd != myPassword || clone > 0 || dns != wanConfigJsonObject.dns || dns1 != wanConfigJsonObject.dns1 || mtu != wanConfigJsonObject.mtu || wanInfoJsonObject.connected == 0) {
...
function intervalTime(btn, btnText, msgText, mode) {
    ...
    interval = setInterval(function () {
        getWanInfoJsonObject();
        if ((wanInfoJsonObject.link == 0 && mode != 4) || wanInfoJsonObject.reason == 19) {
            timeout = 15;
        }
        if (wanInfoJsonObject.reason == 20) {
            noResponse = 20;
        }
        if (timeout > timeoutCount - 1) {
                ...
                if (wanInfoJsonObject.connected == 0) {
                    if (wanInfoJsonObject.link == 0 && mode != 4) {
                        ...
                        if (wanInfoJsonObject.reason > 0) {
                            getMsg(getErrorCode(wanInfoJsonObject.reason));
                        } else if (noResponse !== undefined) {
                            getMsg(getErrorCode(noResponse));
                        } else {
                            getMsg(msgText + '超时！');
                        }
                    }
