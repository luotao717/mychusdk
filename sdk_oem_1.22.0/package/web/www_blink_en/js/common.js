...
var regexp = RegExp(/[\':;*?~`!@#$%^&+={}\[\]\<\\(\),\.\。\，]/);
var MODEL = new Array("Other", "Huawei", "Mi", "Meizu ", "netcore", "Lenovo", "ChangHong", "HTC", "IPHONE", "Samsung", "Nokia", "Sony", "Coolpad", "OPPO", "LG", "ZTE", "VIVO", "Moto", "GIONEE", "K-Touch", "TCL", "PHILIPS", "Dell", "HP", "ASUS", "Toshiba", "Acer", "Hisense", "Tenda", "TPLINK", "OnePlus", "Smartisan", "Realtek", "Tongfang", "Chuyun", "Skyworth", "Konka", "LETV", "Haier", "Sharp", "VMware", "Intel", "GIGABYTE", "Haihua", "Other", "Other", "Other", "Other", "Compal", "Other", "THL", "Other", "Other", "Google", "Tinno", "Pantech", "Other", "Other", "Microsoft", "gee", "Qihoo360", "Mercury", "Hixih", "Ainley", "Fuji Xerox", "D-Link", "NETGEAR", "Alibaba", "MSI", "TCT", "Lemmon", "Gaoke", "Nexus", "Doov", "Pepper", "YX", "Ou Boxin", "Anson", "Ramos", "HMD", "yoord", "Amoi", "Jin Hongsheng", "Epad", "Coship", "Voyager", "T-smart", "ShangFeng", "Lan Boxing", "PHICOMM", "COLORFLY", "Coolgen", "Europe", "Jiayu", "Iuni", "Yitong", "vollo", "Daesung", "Cube", "Benwee", "G-hong", "VINUS", "Qz", "ARCHOS", "Mele", "Horke", "InFocus", "XF", "AUX", "BIRD", "BOWAY", "HASEE", "Acer", "Yingteqi", "Cool treasure", "Ouby", "ONDA", "Zhuo cool", "Fortune", "Quang Tin", "Koobee", "Bo cool", "Witu", "Fm", "GUOMI", "MEEG", "Milan", "NYX", "NOMI", "Tensent", "Subor", "Huaxing", "UBTEL", "neken", "ZEEMI", "MTK", "CISCO", "BLINK");
...
function getErrorCode(type) {
    switch (type) {
        case 19 :
            ...
            break;
        case 20 :
            return "dialing no response!";
            break;
        case 10001:
            return "Get parameter failure!";
            break;
        case 10002:
            return "Input parameter error!";
            break;
        case 10003:
            return "Routing memory shortage!";
            break;
        case 10004:
            return "Added entries already exist!";
            break;
        case 10005:
            return "Deleted entries do not exist!";
            break;
        case 10006:
            return"Add entries are full!";
            break;
        case 10007:
        ...
        case 10008:
            ...
            break;
        case 10009:
            return"No access to account!";
            break;
        case 10010:
            return "Time expired!";
            break;
        default :
            return "Request timeout, error code：" + type;
    }
}
...
function showDHCP() {
    ...
    if (wanConfigJsonObject.dns == '' && wanConfigJsonObject.dns1 == '') {
        if (wanConfigJsonObject.oldmode == undefined) {
            li.each(function () {
                $(this).hide();
                $("#DhcpHightSet").text('Advanced');
            });
        } else if ((wanConfigJsonObject.olddns !== undefined && wanConfigJsonObject.olddns != '') || (wanConfigJsonObject.olddns1 !== undefined && wanConfigJsonObject.olddns1 != '')) {
            li.each(function () {
                $(this).show();
                $("#DhcpHightSet").text('Simple');
            });
        }
    } else {
        li.each(function () {
...
function setTerminalNick(mac, nick) {
    ...
    if (encodeURIComponent(macNick) != nick) {
        $.ajax({
            type: "POST",
            url: actionUrl + "fname=net&opt=host_nick&function=set&mac=" + mac + "&nick=" + nick,
            dataType: "json",
            success: function (data) {
                if (data.error == 0) {
                } else {
...
function setTerminalSpeed(mac, down_speed, up_speed) {
    var macTmp = mac.replace(/:/g, "");
    var macDownSpeed = $("#h_d_" + macTmp).val() == "No Limit" ? "0" : $("#h_d_" + macTmp).val();
    var macUpSpeed = $("#h_u_" + macTmp).val() == "No Limit" ? "0" : $("#h_u_" + macTmp).val();
    //alert(macDownSpeed + "   " + down_speed + "   -----------" + macUpSpeed + "   " + up_speed);
    if (macDownSpeed != down_speed || macUpSpeed != up_speed) {
        $.ajax({
            type: "POST",
            url: actionUrl + "fname=net&opt=host_ls&function=set&mac=" + mac + "&speed=" + down_speed + "&up_speed=" + up_speed,
            dataType: "json",
            success: function (data) {
                if (data.error == 0) {
                } else {
                    ...
                    for (var i = 0; i < terminals.length; i++) {
                        ...
                        if (MODEL != null) {
                            if (MODEL[terminals[i].vendor] === undefined) {
                                MODEL[terminals[i].vendor] == 'Other';
                            } else {
                        ...
                        if (terminals[i].flag.substr(6, 1) == 'T') {
                            ...
                            device_name = 'Chuyun-' + mac_mark.substr(-4, 4);
                        } else {
...
var router = {
    ...
    getVersion: function () {
        $.ajax({
            type: "POST",
            url: actionUrl + "fname=system&opt=firmstatus&function=get&flag=local",
            dataType: "json",
            success: function (data) {
                var jsonObject = data;
                if (jsonObject.error == 0) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (jsonObject.error == 0) {
                    ...
                    $("#netType").html("Access to network type failure!");
                    $("#wanIP").html("failure!");
                }
            }
        });
    ...
    getTerminalLimitSpeed: function () {
        ...
        $.ajax({
            type: "POST",
            url: actionUrl,
            data: "fname=system&opt=host_if&function=get&if=FFFFFFFT",
            async: false,
            dataType: "json",
            success: function (data) {
                var jsonObject = data;
                    ...
                    if (terminals != null && terminals.length > 0) {
                        for (var i = 0; i < terminals.length; i++) {
                            ...
                            if (terminal_speed_map != null) {
                                ...
                                tmp_down_speed = tmp_down_speed == "0" ? "No Limit" : tmp_down_speed;
                                tmp_up_speed = tmp_up_speed == "0" ? "No Limit" : tmp_up_speed;
                                //alert(terminals[i].mac + "   "+ tmp_down_speed + "  " + tmp_up_speed);
                            }
                            if (terminals[i].flag.substr(1, 1) == 'T') {
                                html += '<tr mac="' + terminals[i].mac + '">';
                                html += "<td><div class='tbDiv'><span class='name'><input type='hidden' id='h_nick_" + terminals[i].mac.replace(/:/g, "") + "' value='" + device_name + "'><input type='text' readonly='readonly' class='tbInpt nameInpt' value='" + device_name + "'></span><span class='small'><i class='tbicon-1 " + wifi_flag + "'></i>" + terminals[i].ip + "</span></div></td>";
                            ...
                            if (terminals[i].flag.substr(1, 1) == 'F') {
                                ...
                                html2 += "<td><div class='tbDiv'><span class='name'><input type='hidden' id='h_nick_" + terminals[i].mac.replace(/:/g, "") + "' value='" + device_name + "'><input type='text' readonly='readonly' class='tbInpt nameInpt' value='" + device_name + "'></span><span class='small'><i class='tbicon-1 " + wifi_flag + "'></i>" + terminals[i].ip + "</span></div></td>";
                    ...
                    $(".name").find("input.tbInpt").on("focusout", function () {
                        ...
                        if (interval_flag) {
                            if (getStrLength(trim(terminal_nick)) < 1) {
                                getMsg('Terminal name can not be empty!');
                                terminal_nick = v;
                            }

                            if (getStrLength(terminal_nick) > 31) {
                                getMsg('Terminal name is too long!');
                                terminal_nick = v;
                            }

                            if (/[\':;*?~`!@#$%^&+={}\[\]\<\\(\),\.\。\，]/.test(terminal_nick)) {
                                getMsg('Terminal name cannot contain special characters!');
                                terminal_nick = v;
                            }
                                ...
                                $("#slider_" + mapArray[m].replace(/:/g, "") + "_" + i).slider({
                                    ...
                                    slide: function (event, ui) {
                                        ...
                                        if (ui.value >= (common_total_speed - common_speed_step - common_speed_min)) {
                                            $(this).siblings().find(".limit").val("No Limit");
                                        } else {
    ...
    getWifiAp: function () {
        $.ajax({
            type: "POST",
            url: actionUrl + 'fname=net&opt=wifi_ap&function=get',
            dataType: 'JSON',
            success: function (data) {
                var jsonObject = data;
                if (jsonObject.error == 0) {
