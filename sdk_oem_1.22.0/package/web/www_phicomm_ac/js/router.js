...
//        }
    ...
    $("#wifi_set_confirm_5g").click(function () {
        var type = '_5g';
        wifisetconfirm(type);
    });
    //添加隐藏ssid
...
//    })
    ...
    var chks_5g = $("input[name='day_5g']");
    $("#day0_5g").click(function () {
        if ($(this).attr("checked") == "checked") {
            chks_5g.attr("checked", "checked");
        } else {
            chks_5g.removeAttr("checked");
        }
    });
...
//    })
    ...
    $("#wifi_on").click(function () {
        ...
        $("#panel").show();
        var type = '';
        setWifiTimeOn(type, 1);
    });

    //5g wifi开关
    $("#wifi_on_5g").click(function () {
        $("#wifisetpanel_5g").show();
        $("#rate").show();
        $("#panel_5g").show();
        var type = '_5g';
        setWifiTimeOn(type, 1);
    ...
    $("#wifi_off").click(function () {
        var type = '';
        setWifiTimeOn(type, 0);
    });

    $("#wifi_off_5g").click(function () {
        var type = '_5g';
        setWifiTimeOn(type, 0);
    });
    ...
    $("#wifiTimeOn_5g").click(function () {
        $("#wifiTimeOn_5g").addClass('selected');
        $("#wifiTimeOff_5g").removeClass('selected');
        $("#wf_time_div_5g").show();
    });
    ...
    $("#wifiTimeConfirm_5g").click(function () {
        var type = '_5g';
        setWifiTimeOn(type, 1);
    });
    ...
    $("input[name='channelType']").click(function () {
        var val = $(this).val();
        if (val == 1) {
            $("#check_channel option").remove();
            $("#check_channel").append("<option>149</option><option>153</option><option>157</option><option>161</option>");
        } else {
            $("#check_channel option").remove();
            $("#check_channel").append("<option>1</option><option>2</option><option>3</option><option>4</option><option>5</option><option>6</option><option>7</option><option>8</option><option>9</option><option>10</option><option>11</option><option>12</option><option>13</option>");
        }
    });
...
function setWifiTimeOn(type, enable) {
    var startHour = $("#startHour" + type).val();
    var startMin = $("#startMin" + type).val();
    var endHour = $("#endHour" + type).val();
    var endMin = $("#endMin" + type).val();
    var week = '';
    if ($("input[name=dayAll" + type + "]").attr('checked')) {
        week = '1111111';
    } else {
        $("input[name=day" + type + "]").each(function () {
    ...
    var time_on = 0;
    if ($("#wifiTimeOn" + type).hasClass('selected')) {
        time_on = 1;
    }
//    if (time_on == 1 && oldTimeOn == 1) {
...
//        }
//    }
    wifiSet(type, enable, time_on, week, startHour, startMin, endHour, endMin);
}

function wifisetconfirm(type) {
    var ssid = $("#wf_nameset" + type).val();
    var pwd = $("#wf_pwdset" + type).val();
    var channel = $("#wifiChannel" + type).val();
    var wifiBandwidth = $("#wifiBandwidth" + type).val();
    var hidden_ssid = 0;
    if ($("#hidden_ssid" + type).attr('checked')) {
        hidden_ssid = 1;
    }
    if (ssid == '') {
        getMsg('请输入无线名称！', 1, '#wf_nameset' + type);
        return;
    }
    if (getStrLength(ssid) > 31 || /[\':;*?~`!@#$%^&+={}\[\]\<\\(\),\.\。\，]/.test(ssid)) {
        getMsg('无线名称长度不得超过31位字符或者包含特殊字符！', 1, '#wf_nameset' + type);
        return;
    }

    if (pwd.length > 0) {
        if (pwd.length > 31 || pwd.length < 8) {
            getMsg('密码长度不能超过31位或不能少于8位！', 1, '#wf_pwdset' + type);
            return;
        }
        if (escape(pwd).indexOf("%u") != -1 || /[\'\"{}\[\]]/.test(pwd)) {
            getMsg('密码不能包含中文字符或者特殊字符！', 1, '#wf_pwdset' + type);
            return;
        }
    }
    pwd = encodeURIComponent(pwd);
    setWifiAp(type, ssid, pwd, channel, hidden_ssid, wifiBandwidth);
}