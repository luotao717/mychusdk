...
#ifdef CONFIG_SYS_EVNENT_NETLINK
...
#endif
#ifdef CONFIG_SYS_EVNENT_NETLINK
struct sock *sys_event_nl_sk = NULL;
int g_switch_on_line = MSG_TYPE_OFFLINE;
#endif
int g_switch_intr_no = 0;
...
#endif
#ifdef CONFIG_NF_IGD
		rx_skb->igd_priv.in_vport = rx_ring[rx_dma_owner_idx].rxd_info4.SPORT + 1;
#endif
...
void kill_sig_workq(struct work_struct *work)
{
	...
	int shoud_send_dhcp_sig = 0;
#ifdef CONFIG_SYS_EVNENT_NETLINK
	NDIS_AP_802_11_NETLINK_MSG_INFO *nl_msg_info;
    int len = 0;
	NDIS_AP_802_11_STA_DEV_MSG dev_info;
#endif

#if defined (CONFIG_WAN_AT_P0)
	if(g_switch_intr_no==0) {
		shoud_send_dhcp_sig = 1;
 	}
#elif defined (CONFIG_WAN_AT_P4)
	if(g_switch_intr_no==4) {
		shoud_send_dhcp_sig = 1;
	}
#endif

    if (shoud_send_dhcp_sig){
...
#ifdef CONFIG_SYS_EVNENT_NETLINK
    len = sizeof(NDIS_AP_802_11_NETLINK_MSG_INFO) + sizeof(NDIS_AP_802_11_STA_DEV_MSG);
    nl_msg_info = (NDIS_AP_802_11_NETLINK_MSG_INFO *)kmalloc(len, GFP_ATOMIC);
    if(!nl_msg_info) {
        printk("--->%s, %d, malloc fail\n", __func__, __LINE__);
        return;
    }

	memset(&dev_info, 0, sizeof(NDIS_AP_802_11_STA_DEV_MSG));
	memset(nl_msg_info, 0, sizeof(NDIS_AP_802_11_NETLINK_MSG_INFO));
   
	dev_info.MsgType = g_switch_on_line;
	dev_info.u.Port = g_switch_intr_no;
	dev_info.AccessType = ACCESS_TYPE_WIRE;
    
	memcpy(&nl_msg_info->Body, &dev_info, sizeof(NDIS_AP_802_11_STA_DEV_MSG));
	nl_msg_info->MsgType = NL_MSG_DEV_ONLINE_STATUS;
	nl_msg_info->MsgLen = len;
	
	printk("-------->%s, %d!!!\n", __func__, __LINE__);
	mtk_nl_send_msg(nl_msg_info, nl_msg_info->MsgLen);
    kfree(nl_msg_info);
#endif
...
static void esw_link_status_changed(int port_no, void *dev_id)
{
    ...
    if(reg_val & 0x1) {
#ifdef CONFIG_SYS_EVNENT_NETLINK
        g_switch_on_line = MSG_TYPE_ONLINE;
#endif
...
int __init ra2882eth_init(void)
{
	...
	ret = debug_proc_init();
#if defined(CONFIG_SWCONFIG)
    mt7530_probe(dev_raether, RALINK_ETH_SW_BASE, NULL);
#endif
	dev_raether = dev;
	
#ifdef CONFIG_SYS_EVNENT_NETLINK
	sys_event_nl_sk = init_sys_nl();
#endif
	return ret;
}
...
#ifdef CONFIG_NF_IGD
int __get_link_status(unsigned long *bitmap)
{
	int status;
	int i;
...
#if defined (CONFIG_RALINK_RT6855) || defined(CONFIG_RALINK_RT6855A) || \
    defined (CONFIG_RALINK_MT7620)
    	status = *((volatile u32 *)(RALINK_ETH_SW_BASE +
			       	0x3008 + (i*0x100)));
#elif defined (CONFIG_RALINK_MT7621)
    	mii_mgr_read(31, (0x3008 + (i*0x100)), &status);
#endif
	if (status & 0x1) 
		set_bit(i, bitmap);
	}
