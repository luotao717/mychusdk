...
)
{
	...
	if (hashAlg == 0x0F)
		saRecord->saCmd0.bits.digestLength = 0x0; //null auth
	else
		saRecord->saCmd0.bits.digestLength = 0x3; //Digest Length for ipsec
	...
	while (1)
	{
		...
		if ((done1 == 1) && (done2 == 1))
		{
			if(unlikely(err_sts))
			{
				...
				addrCurrAdapter = (unsigned int *) &(skb->cb[36]);
				currAdapterPtr = (ipsecEip93Adapter_t *)(*addrCurrAdapter);
				printk("\n\n !PE Ring[%d] ErrCode=0x%x! rdn=%d cdn=%d encrypt=%d qlen=%d\n\n", resRingIdx, err_sts, PktCnt,\
