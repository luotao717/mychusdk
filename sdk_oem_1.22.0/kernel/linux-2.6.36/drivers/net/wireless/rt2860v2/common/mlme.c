...
//add PartialScan by jim
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
			...
			(pAd->ApCfg.PartialScanChannelNum == DEFLAUT_PARTIAL_SCAN_CH_NUM))/* pAd->ApCfg.PartialScanChannelNum == DEFLAUT_PARTIAL_SCAN_CH_NUM means that one partial scan is finished */
		{
			...
			{
				printk("[%s] PartialScanBreakTime=[%d], DEFLAUT_PARTIAL_SCAN_BREAK_TIME=[%d] \n", __FUNCTION__, pAd->ApCfg.PartialScanBreakTime, DEFLAUT_PARTIAL_SCAN_BREAK_TIME);
