    ...
    for(j = BSS0; j < pAd->ApCfg.BssidNum; j++)
                {
                        ...
                        DBGPRINT(RT_DEBUG_TRACE, ("RTMPIoctlSet MBSSID[%d] TPMKEntryPeriod:%d\n",
...
*/
		...
		pDst->Validtime = pFrom->Validtime;
    }
    RTMP_INT_UNLOCK(&(pAd->ApCfg.MBSSID[0].TPMKTable.TPMKTableLock), irqFlags);
	...
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
    ...
    DBGPRINT(RT_DEBUG_TRACE, ("RTMPIoctlCuteromIESet->IE:%d, IE len is:%d\n", 
