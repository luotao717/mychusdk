			    ...
			    gfp_t mem_flags)
{
	...
	dir = usb_urb_dir_in(urb) ? DMA_FROM_DEVICE : DMA_TO_DEVICE;
	#if 1 //CCC_DBG
	//strange dma address. force to remap
	if (urb->transfer_dma == 0xffffffff)
		urb->transfer_flags = urb->transfer_flags & ~URB_NO_TRANSFER_DMA_MAP;
	#endif
		...
		struct usb_hcd *primary_hcd)
{
	...
	if (primary_hcd == NULL) {
		//CC: this is not necessary because mutex memory space has been allocated
		//    at the time of hcd allocation
		#if 1
...
static void hcd_release (struct kref *kref)
{
	...
	{
		//CC: this is marked off because mutex memory is allocated at the time
		//    of hcd allocation
