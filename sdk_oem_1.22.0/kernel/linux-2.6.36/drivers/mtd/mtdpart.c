...
#ifdef RALINK_MT7621 
extern uint64_t file_system_start_address;
#endif
   ...
   struct mtd_info *rpart, const struct mtd_partition * part)
{
    ...
    if (master->block_isbad)
    {
	while (master->block_isbad(master,part->offset + bad_block_nu*master->erasesize))
	{
	    printk("0x%llx is a badblock\n",part->offset + bad_block_nu*master->erasesize);
	    bad_block_nu++;   
	}
    }
    if (master->read(master, part->offset+bad_block_nu*master->erasesize, sizeof(hdr), &uimage_len, (void *) &hdr))
        return;
    ...
    if (master->block_isbad)
    {
	for (i = part->offset; len > 0; i += master->erasesize)
	{
	   if (i > master->size) break;
	   if( master->block_isbad(master,i) ) 
	   {	
		bad_block_nu++;
		printk("0x%llx is a badblock\n",i);
	   }
    ...
    ret = split_squashfs(master, rootfs_partition->offset + bad_block_nu*master->erasesize, &rootfs_len);
    ...
    len = uimage_len + rootfs_len + bad_block_nu*master->erasesize;
    unused_partition->size   = part->size - len;
    unused_partition->offset = part->offset + len;
#ifdef RALINK_MT7621
    file_system_start_address = unused_partition->offset;
#endif
