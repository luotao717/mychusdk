 ...
 * AN "AS-IS" BASIS ONLY. MEDreaIATEK EXPRESSLY DISCLAIMS ANY AND ALL WARRANTIES,
...
extern struct mt6575_nand_host *host;
...
int find_empty_page_from_top(int start_addr,struct mtd_info *mtd)
{
	...
	//mt6575_nand_erase(start_addr); //for test
...
void part_init_pmt(struct mtd_info *mtd,u8 *buf)
{
	...
	int i=0;
	int err = 0;
	printk (KERN_INFO "mt6575_part_init_pmt  %s\n", __TIME__);
	...
	while (1)
	{
		...
		part->offset= lastblk;
		lastblk = part->offset+ part->size;
		printk (KERN_INFO "mt6575_part_init_pmt %llx\n",part->offset);
