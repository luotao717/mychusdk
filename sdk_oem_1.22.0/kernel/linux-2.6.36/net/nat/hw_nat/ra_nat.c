...
/*FP to-lan on/off*/
	...
	/* handle the incoming packet which came back from PPE */
	    ...
	    if((pptp_fast_path || l2tp_fast_path) && (skb->len == (124 - 14))){
		...
		pptp_l2tp_fdb_update(PpeParseResult.iph.protocol, addr, FOE_ENTRY_NUM(skb));
...
/*WAN->LAN accelerate*/
	...
	if((iph->protocol == IPPROTO_GRE) || (ntohs(uh->dest)==1701))
	{
		...
		if (DebugLevel >= 1) {
			printk("Lan->Wan PpeFillInL3Info \n");
		}
		/* Set Layer3 Info */
		if (PpeFillInL3Info(skb, foe_entry)) {
			memset(FOE_INFO_START_ADDR(skb), 0, FOE_INFO_LEN);
		...
		if (DebugLevel >= 1) {
			printk("Lan->Wan PpeFillInL4Info \n");
		}
		/* Set Layer4 Info */
		if (PpeFillInL4Info(skb, foe_entry)) {
			memset(FOE_INFO_START_ADDR(skb), 0, FOE_INFO_LEN);
		...
		if (DebugLevel >= 1) {
			printk("Lan->Wan PpeSetForcePortInfo \n");
		}
